...
#sqaure brackets - char sets 
#find if any char belongs in the set
#e.g. 'R[aeiou]' can search for things like Re, Ra, Ri...

#pipe 'cat|dog|cow' seraches for cat or dog or cow

#parenthesis - for grouping '(woof)'  finds a repitition of woof
#another example '(woof|meow)'
#another example '(woof|meow)+' - repeated occurences like woofwoof

#{} - specifying exact occurences
#eg. 'lo{2}' looks for 2 o's after l

#\d hunts down digit chars
# eg. 'xxx xxx-xxxx' '\d{3} \d{3}-\d{4}'

#\w finds word chars
#eg. word containing a no. '\w*\d\w*' - * means any no. of word chars

#\s - seeks out whitespace spaces, tabs , newlines
#'\s+' - any numbers of whitespace

#\D finds any char that is not a digit
# '\D+' would split up anything that is a number if it were used with a split

#\W finds any char that is not a word char - e.g. any special char, spaces etc
#'\W+'

#\S finds any char which is not a white space


#\b - marks the start or end of a word
#eg. find words that start with py '\bpy\w*'

#\B used to enure that the pattern is within a word
#eg. find words that dont begin with Ne
#'\BNe\B'